...
function isLikelyAmount(line) {
    return AMOUNT_PATTERNS.some((pattern) => pattern.test(line));
}
function parseDescription(text, allowFallback = false) {
...
function calculateConfidence(fieldsFound) {
    // Each field contributes equally — 3 fields = high confidence, fewer = degraded
    const scores = { 3: 1.0, 2: 0.7, 1: 0.4, 0: 0.1 };
